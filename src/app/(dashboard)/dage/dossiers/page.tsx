...
const FILTRES: { label: string; valeur: StatutDossier | 'tous' }[] = [
  ...
  { label: 'Enregistrés', valeur: 'receptionne' },
...
export default function DossiersDAGE() {
  const searchParams = useSearchParams()
  const statutParam = searchParams.get('statut') as StatutDossier | null

  const [dossiers, setDossiers] = useState<Dossier[]>([])
  const [filtre, setFiltre] = useState<StatutDossier | 'tous'>(statutParam ?? 'tous')
  const [recherche, setRecherche] = useState('')
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    getDossiers('DAGE').then(d => { setDossiers(d); setLoading(false) })
  }, [])
  ...
  const filtres = dossiers
    ...
    .filter(d =>
      ...
      d.typeDossier.toLowerCase().includes(recherche.toLowerCase())
    )
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Dossiers DAGE</h1>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm mb-4 p-4">
        ...
        <input
          ...
          placeholder="Rechercher par code, direction, type..."
              ...
              {filtres.map(d => (
                <tr key={d.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link href={`/dage/dossiers/${d.id}`} className="font-mono font-semibold text-green-700 hover:underline">
                      {d.code}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{d.typeDossier}</td>
